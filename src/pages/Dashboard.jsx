    ...
    <div className="page-enter col gap-6">
      ...
      >
        ...
        <StatCard
          ...
          value={
            <>
              <AnimatedNumber value={18.4} decimals={1} />{" "}
        ...
        <StatCard
          ...
          subtitle={
            <span>
              <span className="acid">Budget 500 ms</span> · under by 313 ms
            </span>
          }
          accent="acid"
        />
      ...
      <div
        style={{ display: "grid", gridTemplateColumns: "1.55fr 1fr", gap: 16 }}
      >
        <Panel
        ...
        <Panel
          title="Top Avoided Regions This Week"
          right={<span className="tag tag-coral">routed away from</span>}
        >
          <BarChart
            data={regions.map((r) => ({
              label: r.name,
          ...
          >
            Ranked by tons of CO₂ that would have been emitted if traffic had
            defaulted there.
          </div>
        </Panel>
      </div>
        ...
        <div style={{ marginTop: 6 }}>
          {feed.map((e) => (
            ...
            >
              ...
              >
                {e.kind === "pos" ? "+" : e.kind === "neg" ? "−" : " "}
                {Math.abs(e.saved).toFixed(2)} g
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        <div className="row gap-2" style={{ marginBottom: 8 }}>
          ...
          <span className="tag" style={{ color: "var(--text-muted)" }}>
            Brooklyn AI Co · production
          </span>
        </div>
        <h1>Carbon, water, and latency — all green.</h1>
        <p style={{ color: "var(--text-muted)", marginTop: 8, maxWidth: 640 }}>
          A 30-day snapshot of every query you served, the region we routed it
          through, and what your defaults would have cost.
        </p>
      </div>
...
function makeFeedEntry(tick, seeded = false) {
  const idx =
    ((tick % FEED_PROMPTS.length) + FEED_PROMPTS.length) % FEED_PROMPTS.length;
  const prompt = FEED_PROMPTS[idx];
  const route = pickGreenPromptRoute(prompt, { tick });
  const def = pickDefaultRoute(prompt, { tick });
  const baseSaved = def.totalCO2 - route.totalCO2;

  // Realistic distribution: ~70% positive savings, ~15% near-zero (a wash),
  // ~15% small negative — happens when grid intensity moves between the
  // routing decision and the model executing.
  const r = Math.sin(tick * 7.31 + 1.7) * 0.5 + 0.5; // deterministic 0..1
  let saved, kind;
  if (r < 0.15) {
    saved = -(0.05 + Math.abs(baseSaved) * 0.25);
    kind = "neg";
  } else if (r < 0.3) {
    saved = r < 0.225 ? 0 : 0.01;
    kind = "zero";
  } else {
    saved = baseSaved;
    kind = "pos";
